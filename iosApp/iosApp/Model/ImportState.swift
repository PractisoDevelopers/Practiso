...
enum ImportState {
    ...
    case error(model: ImportException, cancel: SendChannelKt, skip: SendChannelKt?, retry: SendChannelKt?, ignore: SendChannelKt?)
    ...
    init(kt: ComposeApp.ImportState) {
        ...
        case .error(let m):
            self = .error(model: m.error, cancel: m.cancel, skip: m.skip, retry: m.retry, ignore: m.ignore)
        case .idle(_):
            self = .idle
